...
void threaded_mergesort() {
    args_1.lower = 0;
    args_1.upper = (((length / 2) - 1) / 2) - 1;
    args_2.lower = args_1.upper + 1;
    args_2.upper = (length / 2) - 1;


    args_3.lower = args_2.upper + 1;
    args_3.upper = args_3.lower + (((length / 2) - 1) / 2) - 1;
    args_4.lower = args_3.upper + 1;
    args_4.upper = length - 1;
    ...
    if(pthread_create(&thread_3, NULL, mergesort, &args_3)) {
        printf("Error creating thread3\n");
        exit(2);
    }
    if(pthread_create(&thread_4, NULL, mergesort, &args_4)) {
